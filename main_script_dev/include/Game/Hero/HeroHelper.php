...
class HeroHelper extends HeroItems
{
    ...
    public function calcTotalHealth($helmet, $body, $shoes)
    {
        return $this->calcHealth() + $this->calcItemHealth($helmet, $body, $shoes);
    }
    ...
    {
        $body = $this->procInput($body);
        if (is_array($body) && in_array($body['type'], [84, 85, 86, 87, 91, 92, 93])) {
            $item = $this->getHeroItemProperties($body['btype'], $body['type']);
    ...
    public function getBandages($bag)
    {
        ...
        if (is_array($bag) && isset($bag['btype']) && in_array($bag['btype'], [7, 8])) {
