...
class BuildingAction
{
    public static function upgrade($kid, $building_field, $levels = 1, $force = false, $modifyPopCP = true)
    {
        ...
        } elseif ($item_id == 10 || $item_id == 11 || $item_id == 38 || $item_id == 39) {
        ...
        } elseif ($item_id == 40 && ($level + $levels) >= 100) {
            $finish = true;
        } elseif ($item_id == 40) {
    ...
    public static function fixUnUpgradedBuildings($kid)
    {
        ...
        foreach ($buildings as $index => $build) {
            if ($build['item_id'] == 0 || $build['level'] > 0) {
                continue;
            }
            if ($index <= 18) {
                continue;
            ...
            if (self::building_upgrade_state($kid, $index)) {
                continue;
            }
