...
class ProcessReturnMovement implements ShouldQueue
{
    use Dispatchable;
    ...
    {
        UnitMovement::query()
            ->missionIn([UnitMovement::MISSION_RETURN])
            ->due()
            ->orderBy('arrives_at')
            ...
            ->each(function (UnitMovement $movement): void {
                $this->processMovement($movement, function (UnitMovement $lockedMovement): array {
                    if ($lockedMovement->target_village_id === null) {
                        throw new RuntimeException('Return movement requires a destination village.');
                    }
