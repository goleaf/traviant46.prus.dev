...
class TroopService
{
    ...
    public function getExpansionUnits($kid)
    {
        ...
        $onTheWay = $db->query("SELECT SUM(u10) AS `settlers`, SUM(u9) AS `chiefs` FROM movement WHERE (kid={$kid} AND mode=0)")
            ->fetch_assoc();
        $settlers += $onTheWay['settlers'];
        $chiefs += $onTheWay['chiefs'];

        $onTheWay = $db->query("SELECT SUM(u10) AS `settlers`, SUM(u9) AS `chiefs` FROM movement WHERE (to_kid=$kid AND mode=1)")
            ->fetch_assoc();
        $settlers += $onTheWay['settlers'];
        $chiefs += $onTheWay['chiefs'];
    ...
    public function getAvailableExpansionTraining($nr, $calc = false)
    {
        ...
        } elseif (($commandCenter = max($village->getTypeLevel(44)))) {
            if ($commandCenter >= 10) {
                $maxslots++;
            }
            if ($commandCenter >= 15) {
