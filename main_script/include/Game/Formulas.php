...
class Formulas
{
    ...
    {
        $basePath = defined('TRAVIAN_ROOT') ? TRAVIAN_ROOT : dirname(__DIR__, 3);
        $buildingsFile = $basePath . '/config/buildings.php';
        $unitsFile = $basePath . '/config/units.php';
        if (!is_file($buildingsFile) || !is_file($unitsFile)) {
            die('Game data configuration files missing.');
        }
        $buildings = require $buildingsFile;
        $units = require $unitsFile;
        if (!is_array($buildings) || !is_array($units)) {
            die('Invalid game data configuration.');
        }
        self::$data = [
            'oases' => [
                2 => [1 => 1],
