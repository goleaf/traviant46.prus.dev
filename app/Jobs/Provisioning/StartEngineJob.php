...
class StartEngineJob implements ShouldQueue
{
    ...
    public function handle(ServerProvisioningService $service): void
    {
        ...
        if ($task === null) {
            Log::warning('Start engine job skipped because the server task no longer exists.', [
                'task_id' => $this->taskId,
            ]);
    ...
    {
        return sprintf('provisioning-start-engine-%d', $this->taskId);
    }
}
