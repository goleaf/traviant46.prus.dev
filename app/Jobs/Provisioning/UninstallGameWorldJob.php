...
class UninstallGameWorldJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(ServerProvisioningService $service): void
    {
        ...
        if ($task === null) {
            Log::warning('Uninstall provisioning job skipped because the server task no longer exists.', [
                'task_id' => $this->taskId,
            ]);
    ...
    {
        return sprintf('provisioning-uninstall-%d', $this->taskId);
    }
}
