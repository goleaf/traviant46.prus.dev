...
class RestartEngineJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(ServerProvisioningService $service): void
    {
        ...
        if ($task === null) {
            Log::warning('Restart engine job skipped because the server task no longer exists.', [
                'task_id' => $this->taskId,
            ]);
    ...
    {
        return sprintf('provisioning-restart-engine-%d', $this->taskId);
    }
}
