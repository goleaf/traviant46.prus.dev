...
### Marketplace & Trading
- **Current state:** Marketplace supply is stored in the `market` table with alliance gating (`aid`), village references (`kid`), resource type integers (`needType`/`giveType`), unsigned BIGINT quantities, and an expiry window (`maxtime`) but no explicit status or timestamps.【F:main_script/include/schema/T4.4.sql†L870-L888】 Hero auctions live in the `auction` table, mixing item metadata (`btype`, `type`, `num`) with bidding counters, current leader tracking (`activeUid`, `activeId`), and security tokens.【F:main_script/include/schema/T4.4.sql†L321-L345】 Bid history is captured in `bids`, retaining only the bidder, auction id, and `outbid`/`del` flags without amounts or audit columns.【F:main_script/include/schema/T4.4.sql†L350-L360】 Farmlist raids rely on `raidlist`, which repeats ten troop columns (`u1`…`u10`) per target and stores distance and target village id alongside the parent list id (`lid`).【F:main_script/include/schema/T4.4.sql†L1253-L1271】
- **Proposed tables:**
  - `market_orders`: replace `market`, referencing `village_id` and `user_id`, storing `offer_resource_type`, `offer_amount`, `request_resource_type`, `request_amount`, `exchange_rate`, `expires_at`, and `visibility` (`public`, `alliance`, `own`). Add `status` (`open`, `fulfilled`, `cancelled`, `expired`) plus `created_at`/`updated_at` timestamps. Create supporting enum/dictionary tables for resource types to avoid magic integers, and add an index on (`status`, `expires_at`).
  - `market_order_events`: append lifecycle and fulfilment data (`event_type`, `payload`, `recorded_at`) to track trades, cancellations, and alliance acceptances. This obviates ad-hoc logging in gameplay code and allows analytics on trade volume.
  - `hero_auctions`: rename `auction`, linking to `users`, `hero_items`, and optionally `villages` (for delivery). Store `starting_bid`, `current_bid`, `buyout_price` (replacing `maxSilver` semantics), `bid_count`, and `ends_at`. Convert legacy security tokens to signed UUIDs and track `status`. Use soft deletes to retain historical pricing.
  - `hero_auction_bids`: replace `bids`, holding `auction_id`, `bidder_id`, `bid_amount`, `is_outbid`, `placed_at`, and optional `silver_balance_snapshot`. Index on (`auction_id`, `placed_at`) for replaying auction history. Reference wallet/accounting tables for reconciliation.
  - `farm_list_targets`: replace `raidlist`, associating each entry with a `farm_list_id`, `target_village_id`, `distance`, and a JSON payload or child table capturing troop compositions (`unit_type_id`, `quantity`). Track `last_sent_at`, `last_result`, and `position` so UI ordering and automation survive. Tie into the new `village_units` schema for type lookups.
- **Migration considerations:** Stage new tables in parallel, backfill from legacy data using ETL scripts that translate resource type integers and troop columns into dictionary-driven values. Maintain views or temporary sync jobs so legacy PHP reads continue during transition. Schedule recalculation of raid distances and market expiry timestamps as part of the migration cutover.
