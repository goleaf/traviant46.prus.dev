...
### Actions
- Game orchestration actions live under `App\Actions\Game`. `CreateVillageAction` now provisions settlements (starter fields, infrastructure, and garrison) while `EnqueueBuildAction`, `TrainTroopsAction`, `CreateMovementAction`, `ResolveCombatAction`, and `ApplyStarvationAction` remain lightweight stubs awaiting their domain logic.
