...
### Actions
- Game orchestration stubs located under `App\Actions\Game` including `CreateVillageAction`, `EnqueueBuildAction`, `CreateMovementAction`, `ResolveCombatAction`, and `ApplyStarvationAction`. `TrainTroopsAction` is now responsible for validating building requirements, deducting resources, and inserting `training_queues` rows while the remaining classes continue to expose constructor-injected repositories awaiting full implementations.
